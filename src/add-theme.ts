...
const fixer: Fixer = {
  ...
  fix: ({ j, api }) => {
    const node = api.closestNode(
      j.CallExpression,
      (n) => j.Identifier.check(n.callee) && n.callee.name === 'styled' && n.arguments.length > 0
    )
    ...
    const edit = () => {
      ...
      const stylesObject =
        node.arguments.length === 2 ? (node.arguments[1] as any) : j.objectExpression([])
